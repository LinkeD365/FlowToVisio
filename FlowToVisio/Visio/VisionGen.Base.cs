...
namespace LinkeD365.FlowToVisio
{
    public partial class FlowToVisioControl : PluginControlBase
    {
            ...
            string relationship, string name)
        {
            ...
            // Add a relationship from the file package to this package part. You can also create
            // relationships between an existing package part and a new part.
        ...
        {
            // Set the inital pidValue to -1, which is not an allowed value. The calling code tests
            // to see whether the pidValue is greater than -1.
            int pidValue = -1;
            ...
            XElement recalcProp = GetXElementByAttribute(props,
                "name", "RecalcDocument");
            // If there is already a RecalcDocument instruction in the Custom File Properties part,
            // then we don't need to add another one. Otherwise, we need to create a unique pid value.
            if (recalcProp != null)
                return pidValue;
            else
            {
                // Get all of the pid values of the property elements and then convert the
                // IEnumerable object into an array.
                ...
                // Increment this id value until a unique value is found. This starts at 2, because
                // 0 and 1 are not valid pid values.
                int id = 2;
                while (pidValue == -1)
        ...
        {
            // Get the Custom File Properties part from the package and and then extract the XML
            // from it.
            PackagePart customPart = GetPackagePart(filePackage,
                "http://schemas.openxmlformats.org/officeDocument/2006/relationships/" +
                "custom-properties");
            XDocument customPartXML = GetXMLFromPart(customPart);
            // Check to see whether document recalculation has already been set for this document.
            // If it hasn't, use the integer value returned by CheckForRecalc as the property ID.
            int pidValue = CheckForRecalc(customPartXML);
            if (pidValue > -1)
            {
                XElement customPartRoot = customPartXML.Elements().ElementAt(0);
                // Two XML namespaces are needed to add XML data to this document. Here, we're using
                // the GetNamespaceOfPrefix and GetDefaultNamespace methods to get the namespaces
                // that we need. You can specify the exact strings for the namespaces, but that is
                // not recommended.
                ...
                // Construct the XML for the new property in the XDocument.Add method. This ensures
                // that the XNamespace objects will resolve properly, apply the correct prefix, and
                // will not default to an empty namespace.
                customPartRoot.Add(
    ...
    XDocument partXML)
        {
            ...
            // packagePart.GetStream().Flush(); Create a new XmlWriter and then write the XML back
    ...
    string relationship)
        {
            // Use the namespace that describes the relationship to get the relationship.
            PackageRelationship packageRel =
                filePackage.GetRelationshipsByType(relationship).FirstOrDefault();
            PackagePart part = null;
            // If the Visio file package contains this type of relationship with one of its parts,
            // return that part.
            if (packageRel != null)
            {
    ...
    PackagePart sourcePart, string relationship)
        {
            // This gets only the first PackagePart that shares the relationship with the
            // PackagePart passed in as an argument. You can modify the code here to return a
            // different PackageRelationship from the collection.
            PackageRelationship packageRel =
            ...
            if (packageRel != null)
            {
                // Use the PackUriHelper class to determine the URI of PackagePart that has the
                // specified relationship to the PackagePart passed in as an argument.
        ...
        private static XDocument GetXMLFromPart(PackagePart packagePart)
        {
            ...
            // Open the packagePart as a stream and then open the stream in an XDocument object.
    ...
    string attributeName, string attributeValue)
        {
            // Construct a LINQ query that selects elements from a group of elements by the value of
            // a specific attribute.
            IEnumerable<XElement> selectedElements =
                from el in elements
                where el.Attribute(attributeName).Value == attributeValue
                select el;
            // If there aren't any elements of the specified type with the specified attribute value
            // in the document, return null to the calling code.
